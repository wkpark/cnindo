...
C     INTEGER OPTION,OPNCLO,HUCKEL,CNDO,INDO,CLOSED,OPEN
      ...
      DATA EL/1HH,2HHe,2HLi,2HBe,1HB,1HC,1HN,1HO,
     & 1HF,2HNe,2HNa,2HMg,2HAl,2HSi,1HP,1HS,2HCl,2HAr,
     & 1HK,2HCa,2HSc,2HTi,1HV,2HCr,2HMn,2HFe,2HCo,2HNi,2HCu/
      END
      ...
      COMMON/PERTBL/EL(MAXELEM)
      COMMON/ORB/ORB(9)
      COMMON/INFO1/CZ(NUMATM),U(MAXORB),ULIM(NUMATM),LLIM(NUMATM),
     & NELECS,OCCA,OCCB
      COMMON/OPTION/OPTION,OPNCLO,HUCKEL,CNDO,INDO,CLOSED,OPEN
      COMMON/AUXINT/A(MAXELEM),B(MAXELEM)
...
C     Pre-definded consts
    ...
    3 FORMAT(5X,46HTHIS PROGRAM DOES NOT DO INDO CALCULATIONS FOR,
     &  51H MOLECULES CONTAINING ELEMENTS HIGHER THAN FLUORINE)
      READ(*,*)
      STOP
    4 CONTINUE
    5 CONTINUE
...
C     ATOMIC INTEGRALS FOR CNDO CALCULATIONS
      COMMON/ARRAYS/S(MAXORB,MAXORB),XX(MAXORB,MAXORB,2)
      COMMON/INFO/NATOMS,CHARGE,MULTIP,AN(NUMATM),N,C(NUMATM,3)
      COMMON/INFO1/CZ(NUMATM),U(MAXORB),ULIM(NUMATM),LLIM(NUMATM),
     & NELECS,OCCA,OCCB
      COMMON/GAB/XXX(MAXORB*5),GAMMA(NUMATM,NUMATM),YYY(MAXORB*2+1)
      COMMON/AUXINT/A(MAXELEM),B(MAXELEM)
      COMMON/OPTION/OPTION,OPNCLO,HUCKEL,CNDO,INDO,CLOSED,OPEN
      DIMENSION MU(MAXELEM),NC(MAXELEM),LC(9),MC(9),E(3)
      DIMENSION T(9,9),PAIRS(9,9),TEMP(9,9),C1(3),C2(3)
      DIMENSION P(MAXORB,MAXORB)
C     Originally by EQUIVALENCE(P(1),Y(1))
      EQUIVALENCE (P(1,1),XX(1,1,1))
      REAL*8 MU,NUM,K1,K2
...
C     DETFRMINATION OF SIZE OF lO  BASIS IN ANDCORE CHARGE CZ
   ...
   50 CONTINUE
      ULIM(I) = N
   60 CONTINUE
C     FILL L ARRAY--=U(J) IDENTIFIES THE ATOM TO WHICH ORBITIL J IS
...
C     COMPUTATION OF 1-CENIER COULOMB INTEGRALS OVER SLATER S FUNCTIONS
      N1 = NC(ANK)
      N2 = NC(ANL)
      K1 = MU(ANK)
      K2 = MU(ANL)
...
C     PROCEDURE FOR CALCULATING REDUCED OVERLAP INTEGRALS
      COMMON/COEFF/Y(9,5,203),Z(17,45)
      COMMON/ARRAYS/S(MAXORB,MAXORB),XX(MAXORB,MAXORB,2)
      COMMON/AUXINT/A(MAXELEM),B(MAXELEM)
      INTEGER   ULIM
      N1=NN1
...
C     * * * * * * * * * * * * * * * * * * * * * * * * * *
  ...
  190 CONTINUE
   ...
   10 A(I+1)=(A(I)*DFLOAT(I)+DEXP(-X))/X
      ...
      SUBROUTINE CMATOUT(N,MATOP)
      IMPLICIT REAL*8(A-H,O-Z)
      PARAMETER (NUMATM=140, MAXORB=350, MAXELEM=29)
      COMMON/ARRAYS/A(MAXORB,MAXORB,3)
      DO 80 M = 1,N,11
      K = M+10
      IF (K.LE.N) GOTO 30
   20 K = N
...
C     OVERLAPS ARE IN MATRIX A, COULOMB INTEGRALS (GAMMA) ARE IN MATRIX G
      COMMON/ARRAYS/A(MAXORB,MAXORB),B(MAXORB,MAXORB),D(MAXORB,MAXORB)
      COMMON/INFO/NATOMS,CHARGE,MULTIP,AN(NUMATM),N,C(NUMATM,3)
      COMMON/INFO1/CZ(NUMATM),U(MAXORB),ULIM(NUMATM),LLIM(NUMATM),
     & NELECS,OCCA,OCCB
      COMMON/GAB/XXX(MAXORB*5),G(NUMATM,NUMATM),Q(MAXORB),YYY(MAXORB),
     & ENERGY
      COMMON/OPTION/OPTION,OPNCLO,HUCKEL,CNDO,INDO,CLOSED,OPEN
      DIMENSION ENEG(MAXELEM,3), BETA0(MAXELEM)
      DIMENSION G1(MAXELEM), F2(MAXELEM)
...
C     ADD V(AB) TO HCORE--CNDO
  ...
  170 CONTINUE
C     EXIT SEGMENT IF ONLY CNDO APPROXMATIONS ARE DESIRED
...
C     OPTIONS CNDO OR INDO
   ...
   10 CONTINUE
      Z = Z+1
      ENERGY = 0.D0
C     TRANSFER CORE HAHILTONIAN TO LOWER TRINAGLE OF A
      DO 20 I=1,N
      A(I,I) = Q(I)
      DO 20 J=I,N
   20 A(J,I) = A(I,J)
...
C     INDO MODIFICATION
   ...
   60 PAA = B(I,I)+ B(I+1,I+1)+ B(I+2,I+2)+ B(I+3,I+3)
     ...
     &F2(K)/50.D0+B(I+J,I+J)*11.D0 * F2(K)/50.D0
  ...
  170 CONTINUE
      IF (Z.LE.IT) GOTO 210
C     SYMMETRIZE F FOR PRINTING (MATRIX A)
  180 DO 190 I=1,N
      DO 190 J=I,N
  190 A(I,J)=A(J,I)
      WRITE(6,200)
...
C     REACTIONS ARE ADDED TO THE F MATRIX TO FORM EITHER THE CNDO OR INDO
C     CORE HAMILTONIAN. THESE ADDITIONS ARE THE INTEGRALS V(AB) FOR CNDO AND
C     CORRECTIONS TO U(I,I) FOR INDO.
      COMMON/ARRAYS/A(MAXORB,MAXORB),B(MAXORB,MAXORB),Q(MAXORB,MAXORB)
      COMMON/GAB/XXX(MAXORB*5),G(NUMATM,NUMATM),FDIAG(MAXORB),
     & PDIAG(MAXORB),ENERGY
      COMMON/INFO/NATOMS,CHARGE,MULTIP,AN(NUMATM),N,C(NUMATM,3)
      COMMON/INFO1/CZ(NUMATM),U(MAXORB),ULIM(NUMATM),LLIM(NUMATM),
     & NELECS,OCCA,OCCB
      ...
      BETA0(16)= -18.150D0
      BETA0(17)= -22.330D0
C     FIND NELECS AND FILL M CORE(DIAGONAL) WITH (I+A)/2
      NELECS = 0
      DO 60 I=1,NATOMS
      NELECS = NELECS + CZ(I)
      LL = LLIM(I)
...
C     PROGRAM).
C     AN UPPER LIMIT OF 25 CYCLES IS INCLUDED (IT)
      COMMON/ARRAYS/A(MAXORB,MAXORB),B(MAXORB,MAXORB),Q(MAXORB,MAXORB)
      COMMON/GAB/XXX(MAXORB*5),G(NUMATM,NUMATM),FDIAG(MAXORB),
     & PDIAG(MAXORB),ENERGY
      COMMON/INFO/NATOMS,CHARGE,MULTIP,AN(NUMATM),N,C(NUMATM,3)
     ...
     & NELECS,OCCA,OCCB
      ...
      F2(8) = 0.266415D0
      F2(9) = 0.31580D0
C     INITIALIZE COUNTER Z AND BEGIN SCF CYCLE AT 10
      Z = 0
      IT = 25
      RHO = 1.D-6
   10  CONTINUE
      Z = Z+1
      ENERGY = 0.0D0
C     TRANSFER CORE HAMILTONIAM TO A
      DO 20 I=1,N
      FDIAG(I) = Q(I,I)
      DO 20 J=1,N
   20 A(I,J) = Q(I,J)
...
C     INDO MODIFICATION
   ...
   70 PAA = B(I,I) + B(I+1,I+1) + B(I+2,I+2) + B(I+3,I+3)
     ...
     &*G1(K)/3.D0+(6.D0*PDIAG(I+J)-2.D0*(PAB- PDIAG(I))) * F2(K)/25.D0
      FDIAG(I+J) = FDIAG(I+J) + (PDIAG(I+J)-(PAB -PDIAG(I)))*F2(K)/5.D0
     &  -PDIAG(I)*G1(K)/3.D0+ (6.D0 * B(I+J,I+J) - 2.D0 * (PAA-B(I,I)))
     &  *F2(K)/25.D0
...
c     TRANSFER F(BETA) TO Q FOR PRINTING
  ...
  260 FORMAT(/,1X,43HEIGENVALUES AND EIGENVECTORS FOR ALPHA SPIN/)
      CALL SCFOUT(1,2)
  270 CONTINUE
C     TRANSFER F(BETA) TO LOWER HALF OF A
      DO 280 I=1,N
      A(I,I) = FDIAG(I)
      FDIAG(I) = 0.0D0
      K = I+1
...
C     PRINTING---ALPHA IN B AND BETA IN A
  ...
  150 FORMAT(15X,7HVALENCE,10X,9HS ORBITAL,10X,9HHYPERFINE)
      WRITE(6,160)
      WRITE(2,160)
  160 FORMAT(10X,50H ELECTRON DENSITY  SPIN DENSITY  COUPLING CONSTANT)
      WRITE(6,170)
      WRITE(2,170)
  170 FORMAT(80X)
      DO 200 I=1,NATOMS
...
C     VEC = RETURNED EIGENVECTORS lN COLUMNS
      COMMON/ARRAYS/A(MAXORB,MAXORB),VEC(MAXORB,MAXORB),X(MAXORB,MAXORB)
      COMMON/GAB/GAMMA(MAXORB),BETA(MAXORB),BETASQ(MAXORB),EIG(MAXORB),
     & W(MAXORB),XYZ(NUMATM*NUMATM+MAXORB*2+1)
C     THE FOLLOWING DIMENSIONED VARIABLES ARE EOUIVLENCED
      DIMENSION P(MAXORB),Q(MAXORB)
      EQUIVALENCE (P(1),BETA(1)),(Q(1),BETA(1))
      DIMENSION IPOSV(MAXORB),IVPOS(MAXORB),IORD(MAXORB)
...
C     PREPARE FOR POSSIBLE BYPASS OF TRANSFORMATION
   ...
   80 SQRTS = DSQRT(S)
      ...
      D = D/TEMP
      B = -SGN * SQRTS
C     D IS FACTOR OR PROPORTIONALTIY. NOT COMPUTE AND SAVE Q VECTOR.
...
C     INITIALIZE AUXILARY TABLES REQUIRED FOR REARRANGING THE VECTORS
      DO 410 J = 1,N
      IPOSV(J) = J
      IVPOS(J) = J
  410 IORD(J) = J
C     TRANSPOSITION SORT TO ORDER THE EIGENVALUES
      M = N
      GOTO 450
  420 DO 440 J=1,M
...
C     THIS ROUTINE IS CALLED IN SCFOUT TO PRINT THE EIGENVALUES M TO K
      COMMON/GAB/XXX(MAXORB*3),EPSILN(MAXORB),
     & YYY(MAXORB*3+NUMATM*NUMATM+1)
